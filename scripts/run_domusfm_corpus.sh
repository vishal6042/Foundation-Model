...
# (3) all collapse fixes (configs/domusfm_corpus_fixed.yaml).
# Logs: runs/domusfm_corpus.log, runs/domusfm_corpus_strongmask.log, runs/domusfm_corpus_fixed.log
set -e
cd "$(dirname "$0")/.."
...
$PY -u -m homefm.baselines.domusfm.run --overlay configs/domusfm_corpus_fixed.yaml > runs/domusfm_corpus_fixed.log 2>&1
